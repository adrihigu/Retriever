...
static class BallArray{
  ...
  BallArray(float cW, float cH, float rCW, float rCH, float cX, float cY, Car c, comunicacion_serial p){
    COURT_WIDTH = cW;
    COURT_HEIGHT = cH;
    ...
    @Override
      public void run(){
        while(requestDataFlag){
          ...
          PVector bBall = new PVector();
          PVector fBall = new PVector();
                ...
                for (int i = 0; i < json.size(); i++) {
                  ...
                  if(ballRecognized != 0? false : true){
                   ...
                   if(balls.get(i+1).getBallColor().equals("BLUE")){
                     ...
                     fBall = balls.get(i+1).getPos();
                   }
                   //println("updateball");
                   //println("BALL ", i+1, ": ", balls.get(i+1).getBallColor(), " Pos: ", balls.get(i+1).getPos(), " Radius: ", balls.get(i+1).getRadius()); 
                }
                //for(int i = 1; i <= balls.size(); i++) {
